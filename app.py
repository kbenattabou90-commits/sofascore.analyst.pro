...
# Tab 1: جدول المباريات الحية والزمنية
with tab_live:
...
# Tab 2: محاكاة الذكاء الاصطناعي العميق والـ xG
with tab_ai:
  ...
  if st.button(t["run_sim"], type="primary"):
    ...
    with res_col2:
      st.info(
          "💡 **تحليل متقدم:** يعتمد هذا التوقع على محاكاة 20,000 سيناريو باستخدام"
          " خوارزميات مون كارلو بناءً على الاستحواذ والضغط الهجومي."
      )

# Tab 3: مؤشر الضغط والزخم الحصري
with tab_momentum:
  st.subheader(t["tabs"][2])
  st.write(
      "📊 تتبع زمني لحظي لنسب الاستحواذ وضغط الأطراف خلال دقائق المباراة التسعين."
  )
  char
